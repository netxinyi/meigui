...
var PCAD = "" +
    ...
    "#天津|和平区,河东区,河西区,南开区,河北区,红桥区,东丽区,西青区,津南区,北辰区,武清区,宝坻区,滨海新区,宁河县,静海县,蓟县" +
    "#上海|黄浦区,徐汇区,长宁区,静安区,普陀区,闸北区,虹口区,杨浦区,闵行区,宝山区,嘉定区,浦东新区,金山区,松江区,青浦区,奉贤区,崇明县" +
    "#重庆|万州区,涪陵区,渝中区,大渡口区,江北区,沙坪坝区,九龙坡区,南岸区,北碚区,綦江区,大足区,渝北区,巴南区,黔江区,长寿区,江津区,合川区,永川区,南川区,潼南县,铜梁县,荣昌县,璧山县,梁平县,城口县,丰都县,垫江县,武隆县,忠县,开县,云阳县,奉节县,巫山县,巫溪县,石柱土家族自治县,秀山土家族苗族自治县,酉阳土家族苗族自治县,彭水苗族土家族自治县" +
...
if (ShowT)PCAD = SPT + "|" + SCT + "#" + PCAD;

var SPLITS = PCAD.split("#");
var PROVINCES = {};
for (var i in SPLITS) {
    var proString = SPLITS[i].split("|");
    var province = proString[0];
    var citys = proString[1].split(",");

    PROVINCES[province] = citys;
}
...
function PCAS() {
    ...
    this.DefP = arguments[2];
    this.DefC = arguments[3];
...
PCAS.SetC = function (PCA) {
    ...
    var value = PCA.SelP.value||SPT;
    var citys = PROVINCES[value];

    PCA.SelC.innerHTML = '';
    for (i = 0; i <= citys.length; i++) {
        var label = citys[i], value = citys[i];
        if (value == SCT) {
            value = ""
        }
        var option = new Option(label, value);
